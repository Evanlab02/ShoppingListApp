...
class TestGetStores(TestCase):
    ...
    def test_get_stores_with_sort(self) -> None:
        """Test the get stores endpoint."""
        result = self.client.get("/api/v1/stores?sort=name&sort_dir=desc")
        self.assertEqual(result.status_code, 200)
        ...
        self.assertEqual(result_json["stores"][1]["name"], self.alt_store.name)
        self.assertEqual(result_json["stores"][1]["store_type"], self.alt_store.store_type)
        self.assertEqual(result_json["stores"][1]["description"], self.alt_store.description)
        self.assertEqual(result_json["stores"][1]["user"]["username"], self.alt_user.username)
