...
class TestItemServiceGetItems(TestCase):
    ...
    async def test_get_items_with_store(self) -> None:
        """Test getting all items for a store."""
        items = await item_service.get_items(store=self.store)
        self.assertEqual(len(items.items), 2)
        self.assertEqual(items.total, 2)
        self.assertEqual(items.page_number, 1)
        self.assertEqual(items.total_pages, 1)
    ...
    async def test_get_items_no_items_store(self) -> None:
        """Test getting all items for a store with no items."""
        items = await item_service.get_items(store=self.alt_store)
        self.assertEqual(len(items.items), 0)
        self.assertEqual(items.total, 0)
        self.assertEqual(items.page_number, 1)
        self.assertEqual(items.total_pages, 1)
